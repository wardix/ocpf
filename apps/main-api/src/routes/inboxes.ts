...
inboxesRoutes.get('/:inbox_id/settings', async (c) => {
  ...
  try {
    ...
    if (!settings) {
      [settings] = await sql`
        INSERT INTO inbox_settings (inbox_id, account_id, auto_assignment_enabled, auto_assignment_algorithm, auto_assignment_max_tickets)
        VALUES (${inboxId}, ${accountId}, false, 'round_robin', 10)
        RETURNING *
      `;
    }
...
inboxesRoutes.put('/:inbox_id/settings', zValidator('json', updateSettingsSchema, (result, c) => {
  ...
  try {
    ...
    const { auto_assignment_enabled, auto_assignment_algorithm, auto_assignment_max_tickets } = c.req.valid('json');

    const [settings] = await sql`
      INSERT INTO inbox_settings (inbox_id, account_id, auto_assignment_enabled, auto_assignment_algorithm, auto_assignment_max_tickets, updated_at)
      VALUES (${inboxId}, ${accountId}, ${auto_assignment_enabled}, ${auto_assignment_algorithm}, ${auto_assignment_max_tickets}, NOW())
